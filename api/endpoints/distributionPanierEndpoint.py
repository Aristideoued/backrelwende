...
@app.route('/addDistributionPanier' ,methods=['GET','POST',"OPTIONS"])
...
def addDistributionPanier():
    ...
    if request.method=='POST':
            ...
            panier=DistributionPanier(taille,total,statut,client,produits,totalReduit)
            db.session.add(panier)
            db.session.commit()
            retour={"code":200,"title":"Ajout d'un DistributionPanier","contenu":"DistributionPanier ajoutée avec succes"}
...
@app.route('/distributionPanierById' ,methods=['GET','POST',"OPTIONS"])
@cross_origin(origin='https://admin.relwende.com', supports_credentials=True, methods=['GET', 'POST', 'OPTIONS'], headers=['Authorization', 'Content-Type'])
@auth.login_required
def getDistributionPanierById():
    if request.method == 'OPTIONS':
        return make_response('', 200)
    if request.method=='POST':
            paniers=[]
            ...
            panier=db.session.query(DistributionPanier).filter(DistributionPanier.id==id)
            ...
            retour={"code":200,"title":"DistributionPanier "+str(id),"contenu":paniers}
            #print(users[0])
            return make_response(jsonify(retour),200)
...
@app.route('/distributionPanierByUser' ,methods=['GET','POST',"OPTIONS"])
@cross_origin(origin='https://admin.relwende.com', supports_credentials=True, methods=['GET', 'POST', 'OPTIONS'], headers=['Authorization', 'Content-Type'])
@auth.login_required
def getDistributionPanierByUser():
    if request.method == 'OPTIONS':
        return make_response('', 200)
    if request.method=='POST':
            paniers=[]
            ...
            retour={"code":200,"title":"DistributionPanier du client "+str(id),"contenu":paniers}
            #print(users[0])
            return make_response(jsonify(retour),200)
...
@app.route('/distributionPaniers' ,methods=['GET','POST',"OPTIONS"])
@cross_origin(origin='https://admin.relwende.com', supports_credentials=True, methods=['GET', 'POST', 'OPTIONS'], headers=['Authorization', 'Content-Type'])
@auth.login_required
def getDistributionPanier():
    if request.method == 'OPTIONS':
        return make_response('', 200)
    if request.method=='GET':
            paniers=[]
            panier = DistributionPanier.query.all()
            ...
            retour={"code":200,"title":"Liste des DistributionPaniers","contenu":paniers}
            #print(users[0])
            return make_response(jsonify(retour),200)
...
@app.route('/delete/distributionPanier' ,methods=['GET','POST',"OPTIONS"])
@cross_origin(origin='https://admin.relwende.com', supports_credentials=True, methods=['GET', 'POST', 'OPTIONS'], headers=['Authorization', 'Content-Type'])
@auth.login_required
def delete_DistributionPanier():
    if request.method == 'OPTIONS':
        return make_response('', 200)
    if request.method=='POST':
            test=False
            ...
            user1=db.session.query(DistributionPanier).filter(DistributionPanier.id==id).first()
            if user1:
                DistributionPanier.query.filter_by(id=id).delete()
                db.session.commit()

                retour={"code":200,"title":"Suppression de  DistributionPanier","contenu":"DistributionPanier supprimée avec succès"}
                return make_response(jsonify(retour),200)
            else :

                retour={"code":401,"title":"Echec de suppression","contenu":"DistributionPanier non trouvée"}
...
@app.route('/update/distributionPanier/totalReduit' ,methods=['GET','POST',"OPTIONS"])
@cross_origin(origin='https://admin.relwende.com', supports_credentials=True, methods=['GET', 'POST', 'OPTIONS'], headers=['Authorization', 'Content-Type'])
@auth.login_required
def update_DistributionPaniers():
    if request.method == 'OPTIONS':
        return make_response('', 200)
    if request.method=='GET':
            ...
            retour={"code":200,"title":"Modification d'une DistributionPanier","contenu":"DistributionPanier modifiée avec succès"}
            return make_response(jsonify(retour),200)
...
@app.route('/update/distributionPanier' ,methods=['GET','POST',"OPTIONS"])
@cross_origin(origin='https://admin.relwende.com', supports_credentials=True, methods=['GET', 'POST', 'OPTIONS'], headers=['Authorization', 'Content-Type'])
@auth.login_required
def update_DistributionPanier():
    if request.method == 'OPTIONS':
        return make_response('', 200)
    if request.method=='POST':
            test=False
...
@app.route('/distributionCart/update', methods=['GET','POST',"OPTIONS"])
@cross_origin(origin='https://admin.relwende.com', supports_credentials=True, methods=['GET', 'POST', 'OPTIONS'], headers=['Authorization', 'Content-Type'])
@auth.login_required
def distribution_cart_update_new():
    if request.method == 'OPTIONS':
        return make_response('', 200)
    if request.method=='POST':
        data = request.get_json()            
        ...
        user = int(data["user"])
        cart=db.session.query(DistributionPanier).filter(DistributionPanier.client==user).first()
        ...
        if cart:
            ...
            retour={"title":"cart not  updated","contenu":"DistributionPanier non trouvé","code":404}
...
@app.route('/distributionCart/product/purge', methods=['GET','POST',"OPTIONS"])
@cross_origin(origin='https://admin.relwende.com', supports_credentials=True, methods=['GET', 'POST', 'OPTIONS'], headers=['Authorization', 'Content-Type'])
@auth.login_required
def distribution_cart_purge():
    if request.method == 'OPTIONS':
        return make_response('', 200)
    if request.method == 'POST':
...
@app.route('/distributionCart/product/quantite', methods=['GET','POST','PUT','UPDATE','DELETE','POST', 'OPTIONS'])
@cross_origin(origin='https://admin.relwende.com', supports_credentials=True, methods=['GET', 'POST', 'OPTIONS'], headers=['Authorization', 'Content-Type'])
@auth.login_required
def quantite_product_in_distribution_cart():
    if request.method == 'OPTIONS':
        return make_response('', 200)
    if request.method == 'POST':         
                if request.data:
...
@app.route('/distributionCart/product/increment', methods=['GET','POST','PUT','UPDATE','DELETE','POST', 'OPTIONS'])
@cross_origin(origin='https://admin.relwende.com', supports_credentials=True, methods=['GET', 'POST', 'OPTIONS'], headers=['Authorization', 'Content-Type'])
@auth.login_required
def increment_product_in_distribution_cart():
    if request.method == 'OPTIONS':
        return make_response('', 200)
    if request.method == 'POST':         
                if request.data:
...
@app.route('/distributionCart/product/decrement', methods=['GET','POST','PUT','UPDATE','DELETE','POST', 'OPTIONS'])
@cross_origin(origin='https://admin.relwende.com', supports_credentials=True, methods=['GET', 'POST', 'OPTIONS'], headers=['Authorization', 'Content-Type'])
@auth.login_required
def decrement_product_in_distribution_cart():
    if request.method == 'OPTIONS':
        return make_response('', 200)
    if request.method == 'POST':         
                if request.data:
...
@app.route('/distributionCart/validation', methods=['GET','POST','PUT','UPDATE','DELETE','POST', 'OPTIONS'])
@cross_origin(origin='https://admin.relwende.com', supports_credentials=True, methods=['GET', 'POST', 'OPTIONS'], headers=['Authorization', 'Content-Type'])
@auth.login_required
def validation_distribution_cart():
    if request.method == 'OPTIONS':
        return make_response('', 200)
    if request.method == 'POST':         
                if request.data:
